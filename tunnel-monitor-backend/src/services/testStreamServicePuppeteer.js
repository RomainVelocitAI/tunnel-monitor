...
class TestStreamServicePuppeteer {
  async streamTest(tunnelId, res) {
    ...
    try {
      ...
      // Initialize browser with Puppeteer
      sendLog('info', 'Initialisation du navigateur (Puppeteer)...');
      ...
      if (ctaButtons.length > 0) {
        ...
        for (let i = 0; i < Math.min(3, ctaButtons.length); i++) {
          const isClickable = await ctaButtons[i].evaluate(el => !el.disabled);
      ...
      logger.error(`Error during Puppeteer test: ${error.message}`);
...
module.exports = new TestStreamServicePuppeteer();